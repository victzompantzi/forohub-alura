...
public class CustomErrorHandler {
  ...
  public ResponseEntity treatError400(MethodArgumentNotValidException e) {
    var errores = e.getFieldErrors().stream().map(DataErrorValidator::new).toList();
    return ResponseEntity.badRequest().body(errores);
  }

  private record DataErrorValidator(String campo, String error) {
    public DataErrorValidator(FieldError error) {
      this(error.getField(), error.getDefaultMessage());
    }
  }

  @ExceptionHandler(CustomExceptionValidator.class)
